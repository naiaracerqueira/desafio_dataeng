...
O particionamento da bronze poderia ser por canal e data de ingestão, já que reprocessamentos seriam realizados por canal: se o parser do Instagram tiver bug, dá para reprocessar source=instagram/ inteiro, não vasculhar todas as datas misturadas com outros canais. Não acredito que 'criador' seja uma boa partição: se temos um volume grande de criadores, as partições começam a ter alta cardinalidade e fragmentam demais os arquivos, degradando a performance.
```
bronze/
├── source=youtube/
...
Na camada Silver, os dados da bronze seriam transformados: extração de informações e criação de colunas a partir do HTML com regex, extração de informações e criação de colunas a partir do Json da API, tipagem dessas colunas, deduplicação das linhas, alguns joins de enriquecimento (com os dados extraídos do Wikipedia), aplicação de dataquality (testes de unicidade e completude). Os dados seriam atualizados por Upsert.
...
Na camada Gold, os dados seriam agredados e modelados e teria duas estratégias para essa camada:
- os dados cadastrais seriam atualizados com `MERGE INTO`/upsert (já que nesse caso o estado atual é suficiente) e não seriam particionados (dado o volume esperado);
- os dados de métricas de engajamento (likes, views, etc) seriam atualizados com snapshot diário/append (para conseguir acessar as variações ao longo do tempo e construir boas soluções enquanto os vídeos estão em alta) e particionados pela data do snapshot.
...
Tabela com valores únicos de `id_criador` para registro de dados cadastrais de cada criador, atualizados eventualmente (upsert). Também teria campos como nome do criador (podendo ser nome civil e/ou artítisco), país de origem do criador, página no wikipedia, nicho.
...
- O valor de `plataforma` deve estar contido em uma lista de valores válidos (youtube, instagram, tiktok, etc).
- Valores de likes, compartilhamentos, views e comentários devem ser maior que 0
...
Exemplos de checks de qualidade:
- Unicidade das chaves das tabelas dimensão: `id_criador`, `id_publicacao`, `id_conta`
- Completude das chaves na tabela fato: `id_criador`, `id_publicacao`, `id_conta`
- O valor de `relevância` deve estar em um range de 0 a 1
